...
func (rpv rsPoolValue) refreshed(conn *DmConnection) (bool, error) {
	...
	tss, err := conn.Access.Dm_build_152(any(rpv.m_TbIds).([]uint32))
	if err != nil {
		return false, err
	}
	rpv.m_lastChkTime = time.Now().Nanosecond()
...
func encodeArgs(stmt *DmStatement, args []driver.Value) ([]any, error) {
	bytes := make([]any, len(args), len(args))
...
func (stmt *DmStatement) executeBatchByRow(args []driver.Value) (err error) {
	...
	for i := 0; i < count; i++ {
		tmpExecInfo, err := stmt.dmConn.Access.Dm_build_109(stmt, args[i].([]any), stmt.preExec || i != 0)
		if err == nil {
