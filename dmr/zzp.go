...
func newTypeData(val any, dataBuf []byte) *TypeData {
...
func (td *TypeData) initTypeData() *TypeData {
	...
	td.m_objRefArr = make([]any, 0)

	return td
}

func (sv TypeData) toStruct(objArr []any, desc *TypeDescriptor) ([]TypeData, error) {
	...
	for i := 0; i < size; i++ {
		...
		default:
			...
			case CLASS, PLTYPE_RECORD:
				tdArr, err := sv.toStruct(objArr[i].([]any), &desc.m_fieldsObj[i])
				if err != nil {
					return nil, err
				}
			...
			case ARRAY, SARRAY:
				tdArr, err := sv.toArray(objArr[i].([]any), &desc.m_fieldsObj[i])
				if err != nil {
					return nil, err
				}
...
func (sv TypeData) toArray(objArr []any, desc *TypeDescriptor) ([]TypeData, error) {
	size := len(objArr)
	retData := make([]TypeData, size)
	for i := 0; i < size; i++ {
		if objArr[i] == nil {
		...
		default:
			...
			case CLASS, PLTYPE_RECORD:
				tdArr, err := sv.toStruct(objArr[i].([]any), desc.m_arrObj)
				if err != nil {
					return nil, err
				}
			...
			case ARRAY, SARRAY:
				...
				tdArr, err := sv.toArray(objArr[i].([]any), desc.m_arrObj)
				if err != nil {
					return nil, err
				}
...
func (sv TypeData) makeupObjToArr(obj any, objDesc *TypeDescriptor) ([]any, error) {
...
func (sv TypeData) toMemberObj(mem any, desc *TypeDescriptor) (*TypeData, error) {
	var bs []byte
	scale := desc.getScale()
	prec := desc.getPrec()
	dtype := desc.getDType()
...
func (sv TypeData) convertLobToBytes(value any, dtype int, serverEncoding string) ([]byte, error) {
	var tmp []byte
	var ret []byte
	if dtype == BLOB {
		lob, ok := value.(DmBlob)
...
func (sv TypeData) objToBytes(data any, desc *TypeDescriptor) ([]byte, error) {
...
func (sv TypeData) addObjToRefArr(out *TypeData, objToAdd any) {
...
func (sv TypeData) bytesToObj(val []byte, out *TypeData, desc *TypeDescriptor) (any, error) {
	var retObj any
	var err error
	if out == nil {
...
func (sv TypeData) objBlobToObj(lob *DmBlob, desc *TypeDescriptor) (any, error) {
...
func (td *TypeData) toJavaArray(arr *DmArray, index int64, l int, dType int) (any, error) {
...
func (td *TypeData) toNumericArray(arr *DmArray, index int64, l int, flag int) (any, error) {
...
func (td *TypeData) toJavaArrayByDmStruct(st *DmStruct) ([]any, error) {
