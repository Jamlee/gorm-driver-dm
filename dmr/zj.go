...
import (
	"bytes"
	"github.com/Jamlee/gorm-driver-dm/dmr/util"
	"io"
	"math/big"
	"strconv"
	"strings"
...
func (G2DB g2db) checkTinyint(val any) error {
	switch v := val.(type) {
	case float64:
...
func (G2DB g2db) checkSmallint(val any) error {
	switch v := val.(type) {
	case float64:
...
func (G2DB g2db) checkInt(val any) error {
	switch v := val.(type) {
	case float64:
...
func (G2DB g2db) checkBigint(val any) error {
	switch v := val.(type) {
	case float64:
...
func (G2DB g2db) checkReal(val any) error {
	switch v := val.(type) {
	case float64:
...
func (G2DB g2db) fromBytes(val []byte, param parameter, conn *DmConnection) (any, error) {
	switch param.colType {
...
func (G2DB g2db) fromString(val string, param parameter, conn *DmConnection) (any, error) {
	switch param.colType {
	case BOOLEAN, BIT:
...
func (G2DB g2db) fromBlob(val DmBlob, param parameter, conn *DmConnection) (any, error) {
	var ret any
	switch param.colType {
	case BINARY, VARBINARY:
...
func (G2DB g2db) fromClob(val DmClob, param parameter, conn *DmConnection) (any, error) {
	var ret any
	switch param.colType {
...
func (G2DB g2db) fromReader(val io.Reader, param parameter, conn *DmConnection) (any, error) {
	var ret any
	switch param.colType {
	case CHAR, VARCHAR2, VARCHAR:
...
func (G2DB g2db) string2Clob(val string, param parameter, conn *DmConnection) (any, error) {
	return G2DB.changeOffRowData(param, Dm_build_890.Dm_build_1103(val, conn.getServerEncoding(), conn), conn.getServerEncoding())
}

func (G2DB g2db) bytes2Blob(val []byte, param parameter, conn *DmConnection) (any, error) {
	return G2DB.changeOffRowData(param, val, conn.getServerEncoding())
}

func (G2DB g2db) clob2Clob(val DmClob, param parameter, conn *DmConnection) (any, error) {
	var clobLen int64
	var err error
...
func (G2DB g2db) blob2Blob(val DmBlob, param parameter, conn *DmConnection) (any, error) {
	var clobLen int64
	var err error
	if clobLen, err = val.GetLength(); err != nil {
		return nil, err
...
func (G2DB g2db) changeOffRowData(paramDesc parameter, paramData []byte, encoding string) (any, error) {
...
func (G2DB g2db) fromObject(mem any, param parameter, conn *DmConnection) ([]byte, error) {
	switch v := mem.(type) {
	case bool:
...
func (G2DB g2db) fromArray(x *DmArray, param parameter, connection *DmConnection) (any, error) {
	var ret any
	var err error
	switch param.colType {
	case SARRAY:
...
func (G2DB g2db) fromStruct(x *DmStruct, param parameter, connection *DmConnection) (any, error) {
	var ret any
	var err error
	switch param.colType {
	case CLASS:
